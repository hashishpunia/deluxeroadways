...
const Footer: React.FC<FooterProps> = ({ details }) => {
  const socialLinks = [
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-20 mb-32">
          ...
          <div className="md:col-span-5">
            <h3 className="text-3xl font-black text-slate-950 mb-10 tracking-tighter">
              {details.name.split(' ')[0]}<span className="text-amber-500">{details.name.split(' ').slice(1).join('')}</span>
            </h3>
            <p className="text-slate-500 text-lg font-medium leading-relaxed max-w-sm mb-10">
              Setting the infrastructure standard for Indian logistics movement since {details.estd}. Reliable. Professional. Scalable.
            </p>
          ...
          <div className="md:col-span-7 grid grid-cols-2 md:grid-cols-3 gap-10">
             ...
             <div>
               ...
               <ul className="space-y-4 text-sm font-bold text-slate-400">
                 ...
                 <li><a href="#about" className="hover:text-slate-950 transition-colors">{details.location.split(',')[0]} Ops</a></li>
               </ul>
             </div>
             ...
             <div className="col-span-2 md:col-span-1">
               ...
               <div className="space-y-6">
                 <a href={`tel:${details.phone}`} className="flex items-center gap-3 text-slate-400 text-sm font-bold group cursor-pointer hover:text-slate-950 transition-colors">
                   <Phone size={16} /> <span>{details.phone}</span>
                 </a>
                 <a href={`mailto:${details.email}`} className="flex items-center gap-3 text-slate-400 text-sm font-bold group cursor-pointer hover:text-slate-950 transition-colors">
        ...
        <div className="pt-10 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6 text-center md:text-left">
          <div className="text-[10px] font-black text-slate-400 tracking-widest uppercase">
            © {new Date().getFullYear()} {details.name}. Verified GST No: {details.gst}
          </div>
