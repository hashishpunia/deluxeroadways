  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row font-sans relative overflow-hidden">
      ...
      <main className="flex-1 min-h-screen overflow-y-auto p-6 md:p-14 pb-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 animate-in fade-in">
            {services.length === 0 ? (
               <div className="col-span-full py-20 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-200">
                 ...
                 <p className="text-slate-400 font-bold">No services found. Click "Add New Service" to begin.</p>
               </div>
            ) : (
              services.map(s => (
          ...
          <div className="space-y-12 animate-in fade-in">
            {testimonials.filter(t => !t.approved).length > 0 && (
              <div className="space-y-6">
                <h4 className="text-xs font-black uppercase tracking-[0.3em] text-amber-500 flex items-center gap-2">
                  <Inbox size={14} /> Inbox (Verification Required)
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {testimonials.filter(t => !t.approved).map(t => (
                    <div key={t.id} className="bg-amber-50/50 p-6 rounded-[32px] border border-amber-100 shadow-sm relative">
                      ...
                      <div className="mt-6 flex gap-3 pt-4 border-t border-amber-100">
                        <button onClick={() => toggleTestimonialStatus(t.id)} className="flex-1 bg-slate-950 text-white py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2">
            ...
            <div className="space-y-6">
              <h4 className="text-xs font-black uppercase tracking-[0.3em] text-slate-400 flex items-center gap-2">
                <CheckCircle2 size={14} /> Live Feedbacks (Public)
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {testimonials.filter(t => t.approved).map(t => (
                  <div key={t.id} className="bg-white p-6 rounded-[32px] border border-slate-200 group relative">
                    ...
                    <div className="mt-6 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => toggleTestimonialStatus(t.id)} className="flex-1 bg-slate-100 text-slate-600 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2">
                        <EyeOff size={14} /> Make Private
                      </button>
                      <button onClick={() => setEditingTestimonial(t)} className="p-3 text-slate-400 hover:text-slate-950 transition-colors bg-slate-50 rounded-xl"><Edit size={16} /></button>
                      <button onClick={() => setTestimonials(testimonials.filter(item => item.id !== t.id))} className="p-3 text-slate-400 hover:text-red-500 transition-colors bg-red-50 rounded-xl"><Trash2 size={16} /></button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 animate-in fade-in">
            <div className="col-span-full bg-white p-8 rounded-[40px] border border-slate-200 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Appears in Navigation and Footer.</p>
              </div>
              <div className="flex items-center gap-6">
                {companyDetails.logo ? <img src={companyDetails.logo} className="h-12 w-auto object-contain rounded" /> : <div className="h-12 w-12 bg-slate-50 rounded border flex items-center justify-center text-slate-200"><ImageIcon size={20} /></div>}
            ...
            {[
              { key: 'heroImage', label: 'Main Hero Banner', desc: 'Primary landing visual (1920x1080 recommended).' },
              { key: 'aboutImage', label: 'About Section Visual', desc: 'Appears alongside the heritage text.' }
            ].map(item => (
          ...
          <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[500] flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in">
              <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                ...
                <button onClick={() => setEditingService(null)} className="p-2 text-slate-400 hover:text-slate-950 transition-all"><X size={32} /></button>
              </div>
              <div className="p-10 space-y-6">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Service Title</label>
                    <input required value={editingService.title} onChange={e => setEditingService({...editingService, title: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold" placeholder="e.g. Heavy Duty Transport" />
                  </div>
                  <div className="space-y-2">
                    ...
                    <select value={editingService.icon} onChange={e => setEditingService({...editingService, icon: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-black uppercase tracking-widest text-[11px]">
                      ...
                      <option value="zap">Express Transit</option>
                    </select>
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Feature Image URL</label>
                  <input value={editingService.image} onChange={e => setEditingService({...editingService, image: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold text-sm" placeholder="https://unsplash.com/..." />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Description</label>
                  <textarea required rows={4} value={editingService.description} onChange={e => setEditingService({...editingService, description: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-bold text-sm" placeholder="Brief overview of the service..." />
                </div>
                <button onClick={handleSaveService} className="w-full bg-slate-950 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-3">
                  <Save size={18} className="text-amber-500" /> Save Fleet Item
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Other tabs and logic remain as specified in App.tsx state... */}
      </main>
    </div>
  );
};
